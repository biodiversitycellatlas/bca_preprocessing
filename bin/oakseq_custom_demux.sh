...
output_dir="${work_dir}/fastq_splitted_i5_3mm_longrun"
mkdir -p "${output_dir}"
...
tmp_dir="${work_dir}/tmp_3mm"
mkdir -p "${tmp_dir}"

# ------------------------------------------------------------------------------
# Extract read IDs from I2 allowing for 3 mismatch, saving non-matches separately
# ------------------------------------------------------------------------------
echo "Extracting read IDs from I2 with up to 3 mismatch allowed..."
...
NR % 4 == 2 {
    matched = "";
    if (hamming($0, bc1) <= 3) matched = bc1;
    else if (hamming($0, bc2) <= 3) matched = bc2;
    else if (hamming($0, bc3) <= 3) matched = bc3;
    else if (hamming($0, bc4) <= 3) matched = bc4;
    if (matched != "") {
