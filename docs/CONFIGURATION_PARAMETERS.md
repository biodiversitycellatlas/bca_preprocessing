...
| `perform_doublet_filtering` | Optional          | Boolean flag to enable or disable consensus doublet detection (Scrublet + scDblFinder, via [Demuxafy](https://demultiplexing-doublet-detecting-docs.readthedocs.io/)) on the raw `.h5ad` before it is passed to CellSweep. Only applies when `ambient_rna_remover` is `"cellsweep"`. Default is `false`. |
