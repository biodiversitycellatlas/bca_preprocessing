...
| Variable               | Required/Optional | Description |
|------------------------|-------------------|-------------|
| `perform_doublet_detection` | Optional          | Boolean flag to enable or disable consensus doublet detection (Scrublet + scDblFinder, via [Demuxafy](https://demultiplexing-doublet-detecting-docs.readthedocs.io/)). Detection runs on the cell-called matrix (STARsolo's `filtered` output, alevin-fry's permit-list output). The calls are handed to CellSweep, which annotates them in `.obs["doublet_status"]` and projects them onto the UMAP comparison plot when cellsweep is enabled. Default is `true`. |
