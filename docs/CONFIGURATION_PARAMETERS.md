...
| `cellfilter_method`    | Optional          | How cells are called, either `"star_solocellfilter"` (each mapper's own cell filter: STARsolo's `--soloCellFilter`, alevin-fry's knee) or `"second_derivative"`. Default set to `"second_derivative"`. Applies to both mappers: it re-calls cells on the most negative second derivative of the cumulative UMI curve, filters the count matrix on that cutoff, and recomputes every cell-dependent statistic in the report on the retained cells. See [Cell calling](#cell-calling). |
