...
include { SATURATION_PLOT as SATURATION_PLOT } from '../modules/saturation_plot'
include { SATURATION as SATURATION_GENEEXT } from '../modules/saturation'
include { SATURATION_PLOT as SATURATION_PLOT_GENEEXT } from '../modules/saturation_plot'
...
workflow QC_mapping_workflow {
    ...
    main:
        ...
        SATURATION_PLOT(MAPPING_STARSOLO.out, SATURATION.out, params.gtf_name)
        ...
        // Combine outputs 
        all_outputs = SATURATION_PLOT.out

        // Optionally include FeatureCounts outputs
        if ( params.perform_featurecounts ) {
            all_outputs = all_outputs.mix(CALC_MT_RRNA.out)
        }

        // Optionally include Kraken outputs
        if ( params.perform_kraken && file(params.kraken_db_path).exists() ) {
            all_outputs = all_outputs.mix(KRAKEN.out)
        }

        // Optionally include Gene Extension outputs
        if ( params.perform_geneext ) {
